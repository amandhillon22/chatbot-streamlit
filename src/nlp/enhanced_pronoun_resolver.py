...
class EnhancedPronounResolver:
    ...
    def __init__(self):
        ...
        self.pronoun_patterns = {
            ...
            # Contextual filtering patterns - NEW
            'contextual_filtering': [
                r'\b(?:which\s+of\s+)?(?:them|these|those)\s+(?:are|is)\s+(?:of\s+)?(?:category|type|status)\s+(\w+)\b',
                r'\b(?:out\s+of\s+)?(?:these|them|those)\s+(?:complaint\s+)?(?:ids?|items?),?\s*(?:whose|which)\s+(?:category|type|status)\s+(?:is|are)\s+(\w+)\b',
                r'\bfilter\s+(?:them|these|those)\s+by\s+(\w+)\b',
                r'\bshow\s+(?:me\s+)?(?:only\s+)?(?:the\s+)?(?:ones|items|complaints)\s+(?:that\s+are|with|of)\s+(?:category|type|status)\s+(\w+)\b',
                r'\b(?:which\s+)?(?:ones|items|complaints)\s+(?:are|have)\s+(?:category|type|status)\s+(\w+)\b',
                r'\bget\s+(?:the\s+)?(?:category|type|status)\s+(?:of|for)\s+(?:them|these|those)\b',
                r'\bshow\s+(?:their|its)\s+(categories?|types?|statuses?)\b'
            ],
        ...
        self.entity_mappings = {
            'vehicles': {
                ...
                'table': 'vehicle_master',  # FIXED: Use vehicle_master instead of crm_site_visit_dtls
                'common_joins': [
            ...
            'complaints': {
                ...
                'detail_fields': ['id_no', 'complaint_date', 'active_status', 'description', 'plant_name', 'category_name', 'category_type_name'],
                'table': 'crm_complaint_dtls',
                'common_joins': [
                    'LEFT JOIN customer_ship_details csd ON cd.cust_id = csd.id_no',
                    'LEFT JOIN crm_complaint_category ccc ON cd.complaint_category_id = ccc.id_no',
                    'LEFT JOIN crm_complaint_category_type ccct ON cd.complaint_type_id = ccct.id_no'
                ],
                'category_fields': {
                    'category': 'ccc.category',
                    'type': 'ccct.category_type_name',
                    'status': 'cd.active_status'
                }
            }
        }
    ...
    def _generate_context_aware_query(self, entity_type: str, last_items: List[Dict], 
        ...
        # Check for contextual filtering (e.g., "which of them are category quality")
        contextual_filter = self._detect_contextual_filtering(original_query, entity_type)
        ...
        # Update the method signature to include contextual_filter
        ...
        # Add contextual filter if detected
        if contextual_filter:
            filter_condition = self._build_contextual_filter_condition(contextual_filter, entity_config, table_alias)
            if filter_condition:
                sql_parts.append(f"AND {filter_condition}")
    ...
    def _detect_contextual_filtering(self, query: str, entity_type: str) -> Optional[Dict]:
        """
        Detect if the query is asking to filter previous results by some criteria
        (e.g., "which of them are category quality")
        """
        query_lower = query.lower()
        
        # Special case: if asking to "show categories/types/status", don't filter - show all
        show_all_patterns = [
            r'\bshow\s+(?:their|its)\s+(categories?|types?|statuses?)\b',
            r'\bget\s+(?:their|its)\s+(categories?|types?|statuses?)\b',
            r'\blist\s+(?:their|its)\s+(categories?|types?|statuses?)\b'
        ]
        
        for pattern in show_all_patterns:
            match = re.search(pattern, query_lower, re.IGNORECASE)
            if match:
                field_requested = match.group(1)
                filter_type = 'category' if 'categor' in field_requested else ('type' if 'type' in field_requested else 'status')
                
                return {
                    'filter_type': 'show_all',
                    'display_field': filter_type,
                    'pattern_matched': pattern
                }
        
        # Check for contextual filtering patterns
        for pattern in self.pronoun_patterns.get('contextual_filtering', []):
            match = re.search(pattern, query_lower, re.IGNORECASE)
            if match:
                filter_value = match.group(1) if match.groups() else None
                
                # Determine the filter type based on pattern and entity type
                filter_type = self._determine_filter_type(query_lower, entity_type)
    ...
    def _determine_filter_type(self, query_lower: str, entity_type: str) -> str:
        """Determine what type of filter is being requested"""
        if 'category' in query_lower:
            ...
            # Default to category for complaints if unclear
            return 'category'
        
        return 'unknown'
    
    def _build_contextual_filter_condition(self, contextual_filter: Dict, entity_config: Dict, table_alias: str) -> Optional[str]:
        """Build the WHERE condition for contextual filtering"""
        filter_type = contextual_filter.get('filter_type')
        filter_value = contextual_filter.get('filter_value')
        
        # If it's a "show_all" request, don't add any filter condition
        ...
        # Get the appropriate column for filtering
        category_fields = entity_config.get('category_fields', {})
        
        if filter_type == 'category' and 'category' in category_fields:
            column = category_fields['category']
            return f"LOWER({column}) = LOWER('{filter_value}')"
        elif filter_type == 'type' and 'type' in category_fields:
            column = category_fields['type']
            return f"LOWER({column}) = LOWER('{filter_value}')"
        elif filter_type == 'status' and 'status' in category_fields:
            column = category_fields['status']
            # Handle status values properly (Y/N for complaints)
            if filter_value.lower() in ['open', 'active']:
                return f"{column} = 'Y'"
            elif filter_value.lower() in ['closed', 'inactive']:
                return f"{column} = 'N'"
            else:
                return f"LOWER({column}) = LOWER('{filter_value}')"
    ...
    def _determine_select_fields(self, entity_type: str, entity_config: Dict, 
                               requested_field: str, query_lower: str, contextual_filter: Optional[Dict] = None) -> str:
        """Enhanced version that handles contextual filtering"""
        
        # If this is a contextual filtering request, include relevant fields
        if contextual_filter:
            base_fields = self._get_base_fields_for_filtering(entity_type, entity_config, contextual_filter)
            return base_fields
    ...
    def _get_base_fields_for_filtering(self, entity_type: str, entity_config: Dict, contextual_filter: Dict) -> str:
        """Get the appropriate fields for contextual filtering display"""
        table_alias = self._get_consistent_alias(entity_config['table'])
        
        if entity_type == 'complaints':
            # For complaints filtering, show ID, basic info, and the filter criteria
            filter_type = contextual_filter.get('filter_type')
            display_field = contextual_filter.get('display_field')  # For show_all cases
            
            base_fields = f"{table_alias}.id_no, {table_alias}.complaint_date, {table_alias}.description"
            
            # Handle show_all cases
            if filter_type == 'show_all':
                if display_field == 'category':
                    base_fields += ", ccc.category"
                elif display_field == 'type':
                    base_fields += ", ccct.category_type_name"
                elif display_field == 'status':
                    base_fields += f", {table_alias}.active_status"
            # Handle specific filtering cases
