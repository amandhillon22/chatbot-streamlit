...
Advanced Embedding Syst                     def _setup_pgvector_database(self):
        ...
        try:
            ...
            raisegvector extension is available
            try:
                cur.execute("CREATE EXTENSION IF NOT EXISTS vector;")
                conn.commit()
                self.use_pgvector = True
                ...
                conn.close()
                conn = get_connection()
                cur = conn.cursor() if pgvector extension is available
            try:
                cur.execute("CREATE EXTENSION IF NOT EXISTS vector;")
                conn.commit()
                self.use_pgvector = True
                ...
                print(f"⚠️ pgvector extension not available: {e}")
                print("🔄 Using fallback mode with JSON storage and scikit-learn similarity")
                self.use_pgvector = FalseSentence Transformers and PostgreSQL with pgvector
...
from src.core.sql import get_full_schema, get_connection
import warnings
...
class SentenceEmbeddingManager:
    ...
    def _create_table_description(self, table_name, columns):
        ...
        # Add column-based semantic context
        semantic_context = []
        for col in columns:
            col_lower = col.lower()
            ...
            elif any(keyword in col_lower for keyword in ['distance', 'km', 'mile']):
    ...
    def _fallback_similarity_search(self, user_query, top_k=5):
        ...
        try:
            ...
            cur.execute("SELECT table_key, description, embedding FROM schema_embeddings;")
            results = cur.fetchall()
            cur.close()
            conn.close()
            
            similarities = []
            for table_key, description, embedding in results:
                if embedding:
                    similarity = cosine_similarity([query_embedding], [embedding])[0][0]
                    similarities.append((table_key, similarity, description))
