...
**🚗 Business Context:** Vehicle tracking utility report table - contains stoppage, overspeeding, distance, and trip report data for vehicles. Links to plant hierarchy through depo_id for regional reporting and operational analysis.
...
| id_no | integer | NO |
| reg_no | character varying | YES |
| from_tm | timestamp without time zone | YES |
| to_tm | timestamp without time zone | YES |
| location | text | YES |
| lat | numeric(10,8) | YES |
| long | numeric(11,8) | YES |
| duration | interval | YES |
| depo_id | integer | YES |
| report_type | character varying(20) | YES |
| created_at | timestamp without time zone | YES |
...
### 🔗 Foreign Keys (Hierarchical Parents)
- depo_id → hosp_master.id_no (Plant/depot information for hierarchy)

### 🔗 Referenced By (Hierarchical Children)
- Links to vehicle_master via reg_no for vehicle details
- Links to hosp_master via depo_id for plant hierarchy (plant → district → zone)

### 🧭 Indexes
- `util_report_pkey`: CREATE UNIQUE INDEX util_report_pkey ON public.util_report USING btree (id_no)
- `util_report_reg_no_idx`: CREATE INDEX util_report_reg_no_idx ON public.util_report USING btree (reg_no)
- `util_report_from_tm_idx`: CREATE INDEX util_report_from_tm_idx ON public.util_report USING btree (from_tm)
- `util_report_report_type_idx`: CREATE INDEX util_report_report_type_idx ON public.util_report USING btree (report_type)
- `sensor2veh_pkey`: CREATE UNIQUE INDEX sensor2veh_pkey ON public.sensor2veh USING btree (sensor2veh_id)
